# This Makefile is for the Sew::Color extension to perl.
#
...
#     ABSTRACT => q[Perl Module for giving rgb values for colours of varous manufactures embroidery thread]
#     AUTHOR => [q[mark winder <markwin@cpan.org>]]
#     BUILD_REQUIRES => {  }
#     NAME => q[Sew::Color]
#     PREREQ_PM => {  }
#     VERSION_FROM => q[lib/Sew/Color.pm]
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Sew/Color.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Sew
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Sew
...
TO_INST_PM = lib/Sew/Color.pm

PM_TO_BLIB = lib/Sew/Color.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/Sew/Color.pm $(INST_MAN3DIR)/Sew::Color.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               Sew-Color' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            1.03' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           Perl Module for giving rgb values for colours of varous manufactures embroidery thread' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - mark winder <markwin@cpan.org>' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl Module for giving rgb values for colours of varous manufactures embroidery thread</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>mark winder &lt;markwin@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Sew/Color.pm blib/lib/Sew/Color.pm 
